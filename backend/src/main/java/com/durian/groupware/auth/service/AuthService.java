...
public class AuthService {
    ...
    // 매번 DB에서 다시 읽어야 한다. /me 와 /login 둘 다 이 메서드로 통일해 응답 필드를 맞춘다.
    public UserInfoResponse getUserInfo(User user) {
        return UserInfoResponse.from(user, resolveDepartmentPath(user.getDepartmentId()));
    }

    // "브랜드팀"만 있으면 어느 부서 소속인지 알 수 없다. 상위로 올라가며 이름을 모아
    // "마케팅부 브랜드팀"처럼 만든다. 맨 위 COMPANY 노드는 모든 사용자에게 똑같이 붙어 정보가 없으므로 뺀다.
